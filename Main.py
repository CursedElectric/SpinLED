...
# Serial settings
serial_port = "COM1"  # Replace with the correct COM port
baud_rate = 576000
...
port_status = "Placeholder text. If you see this something went wrong. Try selecting the correct port for your arduino board and restart the program"
...
def update_colors():
    global red_tap, blue_tap, left_spin_color, right_spin_color, scratch_color, beat_color, beat_boost, spin_animation_speed, max_brightness, tapsizemodifier, tap_speed, brightmod, delay, beat_color_bool, scratch_min, scratch_max, dev_mode
    ...
    spin_animation_speed = float(config.get('Other Effects', 'Spin Animation Speed', fallback = 50)) / 20
    max_brightness = float(config.get('Other Effects', 'Max Brightness' , fallback = 200))
    tapsizemodifier = int(config.get('Other Effects', 'Tap Size Modifier' , fallback = 20)) / 50
    tap_speed = float(config.get('Other Effects', 'Tap Speed' , fallback = 80))
    ...
    dev_mode = (config.get('Options', 'dev mode', fallback = "False"))
...
def on_press(key):
    global blue_match_timer, red_match_timer, spinning_l, spinning_r, scratch, start_spintime, current_value, loop, start_time, livetime, match_rand, holdstart, dev_mode
    if dev_mode == "True":
        try:
            if key.char == '1':  # blue match
                index = int(strip_length / 2)
            ...
            if key.char == '3':  # blue tap
                active_effects.append({
                    "start_index": int(strip_length / 2),
                    "start_time": time.time(),
                    "effect_id": 3,
                    "effect_var": tap_speed #osc_freq
                    })
            if key.char == '4':  # red tap
                active_effects.append({
                    "start_index": int(strip_length / 2),
                    "start_time": time.time(),
                    "effect_id": 4,
            ...
            if effect["effect_id"] == 8:  # Blue Hold
                ...
                if holdstart == True and effect["end"] == False:
                    ...
                    for i in range(max(0, start_index - tap_size), min(strip_length, start_index + tap_size)):
                        ...
                        r, g, b = [int(c * brightness / 255) for c in blue_tap]
                        r = max(x, x + (r - abs((i - start_index) / (tapsizemodifier / 1.3))) / brightmod)  #the number controls the size of the tap. lower == bigger
                        g = max(y, y + (g - abs((i - start_index) / (tapsizemodifier / 1.3))) / brightmod) 
                        b = max(z, z + (b - abs((i - start_index) / (tapsizemodifier / 1.3))) / brightmod) 
                ...
                if holdstart == False or effect["end"] == True:
                    ...
                    for i in range(max(0, start_index - tap_size), min(strip_length, start_index + tap_size)):
                        ...
                        r, g, b = [int(c * brightness / 255) for c in blue_tap]
                        r = r - abs((i - start_index) / (tapsizemodifier / 1.3)) #the number controls the size of the tap. lower == bigger
                        g = g - abs((i - start_index) / (tapsizemodifier / 1.3))
                        b = b - abs((i - start_index) / (tapsizemodifier / 1.3))
