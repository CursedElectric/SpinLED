...
root.geometry("1000x800")  # Increased size to accommodate new slider
root.title("SpinLED")
##
...
if 'dev mode' not in config['Options']:
    config['Options']['dev mode'] = 'False'

with open('config.ini', 'w') as configfile:
    config.write(configfile)

#initializing important variables on startup
Main.strip_length = int(config.get('Options', 'Led Strip Length'))
Main.baud_rate = int(config.get('Options', 'Baud Rate'))
Main.serial_port = (config.get('Options', 'com port'))
...
def write_port_to_config(choice):
    ...
    with open("config.ini", "w") as configfile:
        config.write(configfile)
        Main.serial_port = f"{choice}"
        Main.update_serial = True
    print(f"Stored COM port: {choice}")

def update_beat_color_switch():
    value = beat_color_switch_var.get()
    Main.beat_color_bool
    config.set("Options", "Beat Color", str(value))
    with open("config.ini", "w") as configfile:
        config.write(configfile)

def update_dev_mode_switch():
    value = dev_mode_switch_var.get()
    config.set("Options", "dev mode", str(value))
    with open("config.ini", "w") as configfile:
        config.write(configfile)
...
dev_mode_switch_var = customtkinter.BooleanVar(value=False)

dev_mode_switch = customtkinter.CTkSwitch(master=root,
    text="Dev Mode", variable=dev_mode_switch_var, command=update_dev_mode_switch, onvalue=True, offvalue=False)
dev_mode_switch.grid(column = 2, row = 10) 

dev_label = ctk.CTkLabel(
    root, text = "Dev mode enables the num keys to be pressed to preview note effects")
dev_label.grid(column=2, row=11, sticky=S)

match_label = ctk.CTkLabel(
    root, text = "Match: 1 = Blue, 2 = Red")
match_label.grid(column=2, row=12, sticky=S)

tap_label = ctk.CTkLabel(
    root, text = "Tap: 3 = Blue, 4 = Red")
tap_label.grid(column=2, row=13, sticky=S)

spin_label = ctk.CTkLabel(
    root, text = "Spin: 5 = Left, 6 = Right")
spin_label.grid(column=2, row=14, sticky=S)

Hold_label = ctk.CTkLabel(
    root, text = "Hold: 7 = Blue, 8 = Red")
Hold_label.grid(column=2, row=15, sticky=S)

beat_label = ctk.CTkLabel(
    root, text = "Beat: 9")
beat_label.grid(column=2, row=16, sticky=S)

scratch_label = ctk.CTkLabel(
    root, text = "Scratch: 0")
scratch_label.grid(column=2, row=17, sticky=S)


ports = [port.device for port in serial.tools.list_ports.comports()]
saved_port = config.get("Options", "com port")
default_port = saved_port if saved_port in ports else (ports[0] if ports else "")
port_dropdown = ctk.CTkOptionMenu(root, 
    values=ports,
    command=write_port_to_config)
port_dropdown.set(default_port)
...
tap_speed_value_label = ctk.CTkLabel(
    root, text = f"Tap/Match Speed: {config.get('Other Effects', 'Tap Speed')}")
...
tap_size_value_label = ctk.CTkLabel(
    root, text = f"Tap/Match Size Modifier: {config.get('Other Effects', 'Tap Size Modifier')}")
...
connection_status_var.set(f"Connection Status: {Main.server_status}")

port_status_var = ctk.StringVar()
port_status_var.set(f"Port Status: {Main.port_status}")

def update_connection_status():
    connection_status_var.set(f"Connection Status: {Main.server_status}")
    print(Main.server_status)
...
general_info_label = ctk.CTkLabel(
    root, text = "Please restart program after changing LED Length or Baud Rate!")
